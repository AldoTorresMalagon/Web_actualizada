            ...
            <td>
                <div class="btn-group btn-group-sm">
                    <button class="btn btn-outline-info" title="Ver detalles"
                            data-id="${s.idSubcategoria}" onclick="abrirDetalleSubcat(this.dataset.id)">
                        <i class="bi bi-eye"></i>
                    </button>
...
window.abrirDetalleSubcat = function (id) {
    const s = todasLasSubcats.find(x => String(x.idSubcategoria) === String(id));
    if (!s) return;
    const t = TIPO_INFO[s.tipo] || { label: s.tipo, color: 'secondary', icon: 'bi-tag' };
    const activo = esActivo(s.Estado);
    const body = document.getElementById('detalle-subcat-body');
    body.innerHTML = `
        <div class="row g-3">
            <div class="col-md-6"><small class="text-muted d-block">ID</small><strong>#${s.idSubcategoria}</strong></div>
            <div class="col-md-6"><small class="text-muted d-block">Nombre</small><strong>${s.Descripcion || '—'}</strong></div>
            <div class="col-md-6"><small class="text-muted d-block">Tipo</small>
                <span class="badge bg-${t.color}"><i class="bi ${t.icon} me-1"></i>${t.label}</span>
            </div>
            <div class="col-md-6"><small class="text-muted d-block">Estado</small>
                <span class="badge ${activo ? 'bg-success' : 'bg-danger'}">${activo ? 'Activa' : 'Inactiva'}</span>
            </div>
            <div class="col-md-6"><small class="text-muted d-block">Total productos</small><strong>${s.totalProductos || 0}</strong></div>
            <div class="col-md-6"><small class="text-muted d-block">Fecha de registro</small><small>${FormatUtils.fechaCorta(s.FechaRegistro)}</small></div>
        </div>`;
    new bootstrap.Modal(document.getElementById('detalleSubcatModal')).show();
};